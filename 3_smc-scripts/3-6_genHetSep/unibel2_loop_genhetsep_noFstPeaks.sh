...
#PBS -N ghnFunibel2                    # Name des Batch-Jobs
#PBS -q clmedium                # [(h/GB/nodes):clexpress(2/182/2),clmedium(48/128/78),cllong(100/128/30),clbigmem(100/258/2),clfocean(100/128/4)]
#PBS -o 3.6.2.loop_genHetSep_noFstPeaks_unibel2.stdout
#PBS -e 3.6.2.loop_genHetSep_noFstPeaks_unibel2.stderr
...
for k in {01..24}; do
	...
	if [ -a $WORK/0_data/0_resources/Fst_masks/pFst_lrt_q0.999_peaks_$L.bed.txt.gz ]; then
		...
		> $OUTDIR/$L.unibel2.noFstPeaks.multihetsep.txt
