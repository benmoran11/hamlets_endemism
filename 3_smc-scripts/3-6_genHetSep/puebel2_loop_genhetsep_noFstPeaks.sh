...
#PBS -N ghnFpuebel2                    # Name des Batch-Jobs
#PBS -q clmedium                # [(h/GB/nodes):clexpress(2/182/2),clmedium(48/128/78),cllong(100/128/30),clbigmem(100/258/2),clfocean(100/128/4)]
#PBS -o 3.6.2.loop_genHetSep_noFstPeaks_puebel2.stdout
#PBS -e 3.6.2.loop_genHetSep_noFstPeaks_puebel2.stderr

module load java1.8.0
module load python3.6.2

INDIR=$WORK/3_output/3.5_vcfParser_out/puebel
OUTDIR=$WORK/3_output/3.6_multiHetSep/puebel2
...
for k in {01..24}; do
	...
	if [ -a $WORK/0_data/0_resources/Fst_masks/pFst_lrt_q0.999_peaks_$L.bed.txt.gz ]; then
	    echo $L;
            generate_multihetsep.py --mask=$COVDIR/18152_${L}_coverage.mask.bed.gz \
		--mask=$COVDIR/18176_${L}_coverage.mask.bed.gz \
		--mask=$COVDIR/18178_${L}_coverage.mask.bed.gz \
		--mask=$COVDIR/18180_${L}_coverage.mask.bed.gz \
		...
		$INDIR/segsites_18152_$L.vcf.gz $INDIR/segsites_18176_$L.vcf.gz \
		$INDIR/segsites_18178_$L.vcf.gz $INDIR/segsites_18180_$L.vcf.gz \
		> $OUTDIR/$L.puebel2.noFstPeaks.multihetsep.txt
