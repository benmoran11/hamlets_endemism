...
#PBS -o 3.7.1.crosscoal_genHetSep_noFstPeaks_gemmay.stdout
#PBS -e 3.7.1.crosscoal_genHetSep_noFstPeaks_gemmay.stderr
...
for k in {01..24}; do
		...
		> $OUTDIR/$L.gemmay.noFstPeaks.multihetsep.txt
            echo done
        echo '-----------------------';
	fi
done
